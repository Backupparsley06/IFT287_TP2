...
public class GestionCommodite {
	...
	public List<TupleCommodite> getAll()
			throws IFT287Exception, SQLException
	{
		try
        {
			List<TupleCommodite> commodites = tableCommodites.getCommodites();
            // Commit
            cx.commit();
            return commodites;
        }
            ...
            throws IFT287Exception, SQLException
    {
        try
        {
        	...
        	if (tableInclusionCommodite.existe(iDChambre, iDCommodite))
                throw new IFT287Exception("Commodite déjà inclus : " + iDCommodite);
